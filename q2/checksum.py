...
def parse_tcp_header(data):
    ...
    # Extract TCP checksum (2 bytes) from the TCP header
    tcp_checksum = struct.unpack('! H', data[16:18])[0]
    
    return src_port, dest_port, sequence, ack, flags, data[offset:], tcp_checksum
    ...
    try:
        while True:
            ...
            if eth_proto == 8:  # IPv4
                ...
                if proto == 6:  # TCP
                    src_port, dest_port, sequence, ack, flags, data, tcp_checksum = parse_tcp_header(data)
 
                    if f"0x{tcp_checksum:04x}"=="0x46a4":
                        print(f"Packet {packet_count}: TCP Checksum: 0x{tcp_checksum:04x}")
                        secret_index = 0
                        secret_data = data[secret_index:secret_index + 500]  # Extract 10 bytes after 'secret'
                        print(f"Secret found in packet {packet_count}: {secret_data.decode('utf-8')}") #print(f"Flag found in packet {packet_number}")
